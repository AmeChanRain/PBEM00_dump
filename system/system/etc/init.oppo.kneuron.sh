    ...
    "1")
        ...
        while [ n -lt 3 ]; do
            ...
            if [ -f /data/kneuron/kneuron.ko ]; then
                insmod /data/kneuron/kneuron.ko -f elsa_enable=$elsaenable
            elif [ -f /vendor/lib/modules/kneuron.ko ]; then
                insmod /vendor/lib/modules/kneuron.ko -f elsa_enable=$elsaenable
            else
                insmod /system/lib/modules/kneuron.ko -f elsa_enable=$elsaenable
            fi
            if [ $? != 0 ];then
                n=$(($n+1));
